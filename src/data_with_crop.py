...
def generate_cropped_view(save_preprocess_dir,dataset,objects,date,drive,frames_index,
                          P,Tr_velo_to_cam,R_cam_to_rect,overwrite=False):

    dataset_dir = os.path.join(save_preprocess_dir, 'cropped', date, drive)
    os.makedirs(dataset_dir, exist_ok=True)
    ...
    imgs = []
    for n in frames_index:
        path=os.path.join(dataset_dir,'%05d.bin' % n)
        if overwrite==False and os.path.isfile(path):
            count += 1
            continue
        imgs.append(dataset.rgb[count][0]) #left image
        lidars.append(dataset.velo[count])
        count += 1

    for i in range(len(lidars)):
        crop = align_img_and_pc(imgs[i], lidars[i], P, Tr_velo_to_cam, R_cam_to_rect)
        n=frames_index[i]
        path = os.path.join(dataset_dir, '%05d.bin' % n)
        crop[:,:4].astype('float32').tofile(path)
        print('cropped view {} saved'.format(n))
...
def data_in_single_driver(raw_dir, date, drive, P, Tr_velo_to_cam, R_cam_to_rect, frames_index=None):
    ...
    for i, frames_index in enumerate(frames_idx_chunks):
        ...
        if 1:  ##generate cropped view --------------------
            generate_cropped_view(save_preprocess_dir, dataset, objects, date, drive, frames_index, 
                                  P, Tr_velo_to_cam, R_cam_to_rect, overwrite=True)
...
def preproces(mapping, frames_index):
    ...
    for key in mapping.keys():
        ...
        P, Tr_velo_to_cam, R_cam_to_rect = load_calib_full(os.path.join(cfg.RAW_DATA_SETS_DIR, key, 'calib_full.txt'))
        for driver in drivers_des:
            print('date {} and driver {}'.format(key, driver))
            data_in_single_driver(cfg.RAW_DATA_SETS_DIR, key, driver, P, Tr_velo_to_cam, R_cam_to_rect, frames_index)
