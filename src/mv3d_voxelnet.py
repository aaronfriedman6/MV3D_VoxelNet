...
def top_feature_net(input, anchors, inds_inside, num_bases):
    ...
    with tf.variable_scope('vfe', reuse=tf.AUTO_REUSE) as scope:
        vox_feature = tf.placeholder(
            tf.float32, [None, 35, 7], name='vox_feature')
        vfe1 = VFELayer(32, 'VFE-1')
        vfe2 = VFELayer(128, 'VFE-2')
        mask = tf.not_equal(tf.reduce_max(vox_feature, axis=2, keep_dims=True), 0)
        block = vfe1.apply(vox_feature, mask, input)
        print("AFTER VFE1 SHAPE: {}".format(block.shape))
        block = vfe2.apply(block, mask, input)
        print("AFTER VFE2 SHAPE: {}".format(block.shape))
        block = tf.reduce_max(block, axis=1)
        print("AFTER REDUCE SHAPE: {}".format(block.shape))

    with tf.variable_scope('top-block-1') as scope:
        block = conv2d_bn_relu(block, num_kernels=32, kernel_size=(3,3), stride=[1,1,1,1], padding='SAME', name='1')
